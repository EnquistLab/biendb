...
CREATE INDEX ts_temp_name_id_idx ON ts_temp (name_id);
...
-- Sort order is critical, essentially defines the 
...
SELECT DISTINCT ON (name_id)
	tnrs_scrubbed_id
FROM ts_temp
ORDER BY name_id, overall_score desc, status_rank, source_rank, tnrs_scrubbed_id
) b
