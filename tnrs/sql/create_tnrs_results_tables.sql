-- ---------------------------------------------------------
-- Creates tnrs results tables from parsing and scrubbing of
-- endangered taxon names
-- ---------------------------------------------------------

SET search_path TO :sch;

-- The following schema matches results file from TNRSbatch app,
-- not online TNRS
-- name_id must be text to support compound ids with commas
-- Will be changed to bigint afer parsing of compound ids
DROP TABLE IF EXISTS tnrs_scrubbed;
CREATE TABLE tnrs_scrubbed (
name_id text,
name_submitted text,
overall_score text,
name_matched_id text,
name_matched text,
name_score text,
name_matched_rank text,
author_submitted text,
author_matched text,
author_score text,
name_matched_author text,
name_matched_accepted_family text,
genus_submitted text,
genus_matched text,
genus_score text,
specific_epithet_submitted text,
specific_epithet_matched text,
specific_epithet_score text,
family_submitted text,
family_matched text,
family_score text,
infraspecific_rank text,
infraspecific_epithet_matched text,
infraspecific_epithet_score text,
infraspecific_rank_2 text,
infraspecific_epithet_2_matched text,
infraspecific_epithet_2_score text,
annotations text,
unmatched_terms text,
name_matched_url text,
name_matched_lsid text,
phonetic text,
taxonomic_status text,
accepted_name text,
accepted_name_species text,
accepted_name_author text,
accepted_name_id text,
accepted_name_rank text,
accepted_name_url text,
accepted_name_lsid text,
accepted_name_family text,
overall_score_order text,
highertaxa_score_order text,
source text,
warnings text
);   

-- Final TNRS results
-- Combines and interprets tnrs_parsed and tnrs_scrubbed
DROP TABLE IF EXISTS tnrs;
CREATE TABLE tnrs (
tnrs_id bigserial primary key,
name_id bigint,
name_submitted_verbatim text,
name_submitted text,
name_submitted_rank text,
family_submitted text,
genus_submitted text,
specific_epithet_submitted text,
author_submitted text,
bien_taxonomy_id bigint,
taxon_id text,
family_taxon_id text,
genus_taxon_id text,
species_taxon_id text,
family_matched text,
name_matched text,
name_matched_author text,
name_matched_rank text,
tnrs_name_matched_score numeric,
tnrs_warning text,
matched_taxonomic_status text,
annotations text,
unmatched_terms text,
scrubbed_taxonomic_status text,
scrubbed_family text,
scrubbed_genus text,
scrubbed_specific_epithet text,
scrubbed_species_binomial text,
scrubbed_taxon_name_no_author text,
scrubbed_taxon_canonical text,
scrubbed_author text,
scrubbed_taxon_name_with_author text,
scrubbed_species_binomial_with_morphospecies text,
sources text
);

