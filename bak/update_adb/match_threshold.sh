...
#########################################################################
# Purpose: Remove TNRS name matching and resolution results from vfoi for names with match scores below the threshold value (currently 0.53). 
#
...
while read currid; do
	...
	PGOPTIONS='--client-min-messages=warning' psql -U $user -d $db_private --set ON_ERROR_STOP=1 -q -v sch="$sch" -v vfoi="$vfoi" -v match_threshold=$match_threshold -v fk_tnrs_id=$currid -f $DIR_LOCAL/sql/purge_name.sql
