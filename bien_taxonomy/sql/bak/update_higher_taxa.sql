-- -----------------------------------------------------
-- Populates higher taxon columns in bien_taxonomy_dev
-- 
...
CREATE INDEX bien_taxonomy_updated_idx on bien_taxonomy_dev(updated);
...
ALTER TABLE bien_taxonomy_dev DROP COLUMN updated;

-- Replace empty strings with nulls, if any
UPDATE bien_taxonomy_dev SET "order"=NULL where "order"='';
UPDATE bien_taxonomy_dev SET "superorder"=NULL where "superorder"='';
UPDATE bien_taxonomy_dev SET "subclass"=NULL where "subclass"='';
UPDATE bien_taxonomy_dev SET "class"=NULL where "class"='';
UPDATE bien_taxonomy_dev SET "division"=NULL where "division"='';

-- Create final indexes
DROP INDEX IF EXISTS bien_taxonomy_order_idx;
CREATE INDEX bien_taxonomy_order_idx on bien_taxonomy_dev("order");

DROP INDEX IF EXISTS bien_taxonomy_superorder_idx;
CREATE INDEX bien_taxonomy_superorder_idx on bien_taxonomy_dev("superorder");

DROP INDEX IF EXISTS bien_taxonomy_subclass_idx;
CREATE INDEX bien_taxonomy_subclass_idx on bien_taxonomy_dev("subclass");

DROP INDEX IF EXISTS bien_taxonomy_class_idx;
CREATE INDEX bien_taxonomy_class_idx on bien_taxonomy_dev("class");

DROP INDEX IF EXISTS bien_taxonomy_division_idx;
CREATE INDEX bien_taxonomy_division_idx on bien_taxonomy_dev("division");
