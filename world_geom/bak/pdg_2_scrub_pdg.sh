...
#  
# Notes: 
#	1. Alternative version of pdg_2_scrub.sh. Dumps pdg-related columns
#		to separate table (pdg) and scrub that table instead. 
#	2. Must update original tables separately after running this 
#		script (remember to strip indexes then restore when done).
#
...
# Extract the raw pdg input to table pdg
echoi $e -n "-- Creating table pdg..."
PGOPTIONS='--client-min-messages=warning' psql -U $user -d $db_private --set ON_ERROR_STOP=1 -q -v sch=$dev_schema -f $DIR_LOCAL/sql/create_pdg.sql
source "$DIR/includes/check_status.sh"	

echoi $e -n "-- Inserting raw values..."
PGOPTIONS='--client-min-messages=warning' psql -U $user -d $db_private --set ON_ERROR_STOP=1 -q -v sch=$dev_schema -f $DIR_LOCAL/sql/populate_pdg.sql
source "$DIR/includes/check_status.sh"	
