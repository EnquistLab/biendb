...
: <<'COMMENT_BLOCK_1'
	...
	PGOPTIONS='--client-min-messages=warning' psql -U $user -d $db_private --set ON_ERROR_STOP=1 -q -v sch=$dev_schema -v tbl=$curr_tbl -v id=$pk_temp -v pkey_name=$pkey_name -f $DIR_LOCAL/sql/add_integer_pk.sql
...
COMMENT_BLOCK_1
	...
	pkey_name=$curr_tbl"_pkey"
	PGOPTIONS='--client-min-messages=warning' psql -U $user -d $db_private --set ON_ERROR_STOP=1 -q -v sch=$dev_schema -v tbl=$curr_tbl -v col=$pk_temp -v pkey_name=$pkey_name -f $DIR_LOCAL/sql/drop_column.sql
	source "$DIR/includes/check_status.sh"	
