...
PGOPTIONS='--client-min-messages=warning' psql -U $user -d public_vegbien --set ON_ERROR_STOP=1 -q -f $DIR_LOCAL/sql/bien_4.2.2_fix_morphospecies_public_db.sql
source "$DIR/includes/check_status.sh"  

echoi $e -n "Private database..."
PGOPTIONS='--client-min-messages=warning' psql -U $user -d vegbien --set ON_ERROR_STOP=1 -q -f $DIR_LOCAL/sql/bien_4.2.2_fix_morphospecies_private_db.sql
source "$DIR/includes/check_status.sh"  
