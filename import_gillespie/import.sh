...
echoi $e -n "-- Adding index, primary keys and foreign keys..."
...
echoi $e -n "-- plot_metadata_staging..."
tbl_plot_metadata=$src"_plot_descriptions_raw"
PGOPTIONS='--client-min-messages=warning' psql -U $user -d $db_private --set ON_ERROR_STOP=1 -q -v sch=$dev_schema -v tbl_plot_metadata=$tbl_plot_metadata -v src=$src -f $DIR_LOCAL/sql/load_staging_plot_metadata.sql
source "$DIR/includes/check_status.sh"
...
PGOPTIONS='--client-min-messages=warning' psql -U $user -d $db_private --set ON_ERROR_STOP=1 -q -v sch=$dev_schema -v src=$src -v limit="$sql_limit" -f $DIR_LOCAL/sql/load_staging_vfoi.sql
source "$DIR/includes/check_status.sh"

echoi $e -n "-- analytical_stem_staging..."
tbl_raw=$src"_plot_data_raw"
# Insert a complete record for each stem
for ((fldno=1; fldno<=$dbhflds; fldno++)); do
   #echo $fldno
   dbh_fld="dbh"$fldno
	PGOPTIONS='--client-min-messages=warning' psql -U $user -d $db_private --set ON_ERROR_STOP=1 -q -v sch=$dev_schema -v tbl_raw=$tbl_raw -v dbh_fld=$dbh_fld -v limit="$sql_limit" -f $DIR_LOCAL/sql/load_staging_analytical_stem.sql   
done
source "$DIR/includes/check_status.sh"
...
echoi $e -n "-- analytical_stem_staging...deleting null dbh records..."
PGOPTIONS='--client-min-messages=warning' psql -U $user -d $db_private --set ON_ERROR_STOP=1 -q -v sch=$dev_schema -f $DIR_LOCAL/sql/analytical_stem_staging_delete_null_dbh.sql
source "$DIR/includes/check_status.sh"
