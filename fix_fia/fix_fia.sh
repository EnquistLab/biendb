...
#########################################################################
# Automatic pipeline version of hotfix_4.0.3.sh
#
# Purpose: fixes FIA plot codes by appending state and county
#
# Notes:
#	1. Only updates vfoi, not astem, as plot codes transferred from vfoi
#
# Requirements:
# 	1. Complete political division tables from gnrs. Must be run after  
# 		module poldiv_tables/
...
PGOPTIONS='--client-min-messages=warning' psql -U $user -d $curr_db --set ON_ERROR_STOP=1 -q -v sch=$curr_sch -f $DIR_LOCAL/$hf_dir/fix_fia_0_index_vfoi.sql
source "$DIR/includes/check_status.sh"	

echoi $e -n "- Creating table fia_plot_codes..."
PGOPTIONS='--client-min-messages=warning' psql -U $user -d $curr_db --set ON_ERROR_STOP=1 -q -v sch=$curr_sch -f $DIR_LOCAL/$hf_dir/fix_fia_1_create_fia_plot_codes.sql
source "$DIR/includes/check_status.sh"	

echoi $e -n "- Creating table vfoi_dev with revised FIA plot codes..."
PGOPTIONS='--client-min-messages=warning' psql -U $user -d $curr_db --set ON_ERROR_STOP=1 -q -v sch=$curr_sch -f $DIR_LOCAL/$hf_dir/fix_fia_2_fix_fia_plot_codes.sql
source "$DIR/includes/check_status.sh"	

echoi $e -n "- Create new version of plot_metadata with updated plot codes..."
PGOPTIONS='--client-min-messages=warning' psql -U $user -d $curr_db --set ON_ERROR_STOP=1 -q -v sch=$curr_sch -f $DIR_LOCAL/$hf_dir/fix_fia_3_update_plot_metadata.sql  > /dev/null >> $tmplog
source "$DIR/includes/check_status.sh"	
...
PGOPTIONS='--client-min-messages=warning' psql -U $user -d $curr_db --set ON_ERROR_STOP=1 -q -v sch=$curr_sch -f $DIR_LOCAL/$hf_dir/fix_fia_5_cleanup.sql
source "$DIR/includes/check_status.sh"	
