...
#	sources CVS, NVS and Madidi in legacy table vfoi
#
# Notes:
#	1. Only updates vfoi; poldiv info transferred to plot_metadata and
#		analytical_stem in later steps
# 	2. Must run before generating table plot_metadata
#
...
PGOPTIONS='--client-min-messages=warning' psql -U $user -d $curr_db --set ON_ERROR_STOP=1 -q -v sch=$curr_sch -f $DIR_LOCAL/sql/populate_poldivs.sql
source "$DIR/includes/check_status.sh"	
