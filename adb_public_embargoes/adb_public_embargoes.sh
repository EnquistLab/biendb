...
#########################################################################
# Purpose:	Applies all endangerec taxon and dataset-species embargoes
# 	to the public database.
#
# WARNING: MUST strip indexes from major tables before running this
#	scripts, otherwise may hang the server for days!
#
...
# Replace with 'filter_rainbio.sql' when data providers have been disambiguated
echoi $e -n "-- rainbio..."
...
echoi $e -n "- Removing temporary indexes..."
PGOPTIONS='--client-min-messages=warning' psql -U $user -d $db_public --set ON_ERROR_STOP=1 -q -v sch=$dev_schema -f $DIR_LOCAL/sql/drop_temporary_indexes.sql
source "$DIR/includes/check_status.sh"

######################################################
# Trait species-count embargo
######################################################

echoi $e -n "- Removing traits with <$sp_count_min species..."
