...
if [ "$e" == "false" ] || [ "$verbose" == "false" ]; then
	...
	# Verbose
	sudo -u postgres PGOPTIONS='--client-min-messages=warning' psql -t -A -d $db -q -c "select format('vacuum analyse verbose %I.%I;', n.nspname::varchar, t.relname::varchar) FROM pg_class t JOIN pg_namespace n ON n.oid = t.relnamespace WHERE t.relkind = 'r' and n.nspname::varchar = '$sch' order by 1" | sudo -u postgres psql -U postgres -d $db 
fi
