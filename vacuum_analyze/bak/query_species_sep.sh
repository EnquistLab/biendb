...
#########################################################################
# Purpose: Runs species queries on analytical database
#
# NOTE: Count of observation are for each query separately, not
#		cumulatively
#
...
# include WHERE
base_where="WHERE scrubbed_species_binomial IS NOT NULL AND higher_plant_group IN ('bryophytes', 'ferns and allies','flowering plants','gymnosperms (conifers)', 'gymnosperms (non-conifer)') AND is_location_cultivated IS NULL AND is_invalid_latlong=0 "
...
PGOPTIONS='--client-min-messages=warning' psql -U $user -d $db_private --set ON_ERROR_STOP=1 -q -v sch="$sch" -v check_tbl="$check_tbl" -v sql_where="$sql_where" -v name_where="$name_where" -f $DIR_LOCAL/sql/species_observation_counts_sep_create.sql
...
for where in $wheres; do
	...
	sql_where=$base_where" "$sql_where_part
	echoi $e -n "Filter:'${name_where}'..."
	#echoi $e "Filter: '${sql_where_part}'"
	#echoi $e "Full filter: '${sql_where}'"
	PGOPTIONS='--client-min-messages=warning' psql -U $user -d $db_private --set ON_ERROR_STOP=1 -q -v sch="$sch" -v check_tbl="$check_tbl" -v sql_where="$sql_where" -v name_where="$name_where" -f $DIR_LOCAL/sql/species_observation_counts_sep_insert.sql
	source "$DIR/includes/check_status.sh"	
	IFS=$'\n'
done
IFS=$IFSbak
...
PGOPTIONS='--client-min-messages=warning' psql -U $user -d $db_private --set ON_ERROR_STOP=1 -q -v sch="$sch" -f $DIR_LOCAL/sql/species_observation_counts_sep_crosstab.sql
source "$DIR/includes/check_status.sh"	

echoi $e -n "Indexing crosstab table..."
PGOPTIONS='--client-min-messages=warning' psql -U $user -d $db_private --set ON_ERROR_STOP=1 -q -v sch="$sch" -f $DIR_LOCAL/sql/species_observation_counts_sep_crosstab_index.sql
source "$DIR/includes/check_status.sh"	
